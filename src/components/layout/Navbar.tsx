...
export default function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-white/10 bg-black/50 backdrop-blur-md">
      <div className="mx-auto max-w-6xl px-5 sm:px-8">
        <div className="flex h-16 items-center justify-between gap-4">
          ...
          <div className="hidden items-center gap-3 md:flex">
            <Link href="/tickets" aria-label={t.nav.tickets} className="rounded-md p-2 text-white/60 transition-colors hover:bg-white/10 hover:text-white">
            ...
            {loading ? (
              ...
              <>
                ...
                <form action={logout}>
                  <button type="submit" className="rounded-md p-2 text-white/60 transition-colors hover:text-red-400" title={t.nav.logoutTitle}>
      ...
      {isOpen && (
        <div className="border-t border-white/10 bg-black/50 md:hidden">
          <div className="space-y-1 px-5 py-4">
            {navLinks.map((link) => (
              <Link
                ...
                className="block rounded-md px-4 py-3 text-sm font-bold uppercase tracking-wider text-[var(--muted)] hover:bg-white/10 hover:text-white"
              >
                {link.label}
              </Link>
            ))}
            <div className="mt-4 border-t border-[var(--card-border)] pt-4">
              ...
              {user ? (
                <div className="flex items-center gap-3">
                  <Link href="/dashboard" onClick={() => setIsOpen(false)} className="flex flex-1 items-center justify-center gap-2 rounded-md border border-white/20 py-2.5 text-sm">
                    <User className="h-4 w-4" />{displayName}
                  </Link>
                  <form action={logout}>
                    <button type="submit" className="rounded-md border border-white/20 p-2.5 text-white/50">
