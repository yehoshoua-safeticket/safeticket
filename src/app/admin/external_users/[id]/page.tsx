...
export default function UserDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [role, setRole] = useState<UserRole>('external_user');
  ...
  const ROLE_OPTIONS: { value: UserRole; label: string }[] = [
    { value: 'external_user', label: t.roles.external_user },
  ...
  async function handleSave() {
    ...
    const res = await fetch('/api/admin/users', {
      ...
      body: JSON.stringify({ userId: id, full_name: fullName, email, phone, role, verification_status: verificationStatus }),
    });
    setSaving(false);
    if (!res.ok) {
  ...
  async function handleDelete() {
    ...
    if (!res.ok) {
      ...
      router.push('/admin/external_users');
    }
  }
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-8 sm:px-6">
      <div className="mb-8 flex items-center gap-4">
        <button
          onClick={() => router.push('/admin/external_users')}
      ...
      {loading ? (
        ...
        <div className="overflow-hidden rounded-xl border border-[var(--card-border)] bg-[var(--card)]">
          <div className="divide-y divide-[var(--card-border)]">
            ...
            <div className="flex items-center gap-4 px-5 py-4">
              <span className="w-32 shrink-0 text-sm text-[var(--muted)]">{t.admin.userDetail.fieldVerification}</span>
              <select
